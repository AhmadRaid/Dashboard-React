...
const ClientOfferPriceFields = (props: OfferPriceFieldsProps) => {
    ...
    // البحث التلقائي مع إلغاء الطلبات المتكررة
    const debouncedSearch = useCallback(
        debounce((clientIdentifier: string) => {
            if (!clientIdentifier) {
            ...
            apiSearchClients(clientIdentifier)
                .then((res) => {
    ...
    const handleSearchChange = (value: string) => {
        // إلغاء البحث السابق إذا كان موجودًا
        if (searchTimeoutRef.current) {
            clearTimeout(searchTimeoutRef.current)
        }

        // إذا كانت القيمة فارغة، تنظيف النتائج فورًا
        if (!value) {
            setClientExists(true)
            setHasSearched(false)
            setOrderSelected(false)
        ...
        // البحث بعد تأخير 300 مللي ثانية
            ...
            >
                <div className="flex gap-2 items-start">
                            ...
                            {({ field, form }: FieldProps) => (
                                <>
                                    <Input
                                        {...field}
                                        size="sm"
                                        placeholder="ابدأ بكتابة رقم الهاتف أو اسم العميل للبحث التلقائي"
                                        onChange={(e) => {
                                            field.onChange(e)
                                        ...
                                        onBlur={() => {
                                            // إخفاء النتائج بعد تأخير بسيط لمنع التداخل مع النقر
                                            setTimeout(() => {
                                                setShowResults(false)
                                            }, 200)
                                        }}
                    ...
                    <Button
                        ...
                        icon={<HiSearch />}
                        onClick={() => debouncedSearch(values.clientSearch)}
                    >
                        بحث
                    </Button>
                </div>
